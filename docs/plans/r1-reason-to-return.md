...
- [ ] **R1.7 Territories opt-in.** A `territory` flag in the country table (Antarctica, French Southern Territories, Greenland, Puerto Rico, Western Sahara, French Guiana and the like), a persisted `includeTerritories` setting defaulting to off, and one shared scope computation on `GameApi` that every count and picker uses. The Antarctica continent chip disappears when territories are off.
