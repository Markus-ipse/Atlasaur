...
- [ ] **R1.4 First-run welcome.** One screen, shown once (`atlasaur:seenWelcome`): what Atlasaur is in a sentence, then three doors: start with the big ones (all continents, introduction order does the rest), pick a region (continent chips as a choice), test me (a `test` round). Replaces the current Study intro.
- [ ] **R1.5 Installable.** Web app manifest, icons, a minimal service worker that precaches the built assets and serves them offline. No push, no background sync. Confirm the GitHub Pages subpath works with `base: "./"`.
- [ ] **R1.6 Small countries on a phone.** Auto-frame the prompt's continent when a Name → Click card is drawn on a narrow viewport. Minimum hit area around tiny features. A one-time "pinch to zoom" hint when the target is under about 12 px on screen.
...
- Antarctica and the French Southern Territories leave the default pool behind an opt-in territories chip (R1.4, where the continent chips are reframed).
