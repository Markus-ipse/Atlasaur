...
- **The Study summary mixes scopes without saying so.** In
  `src/components/SessionSummary.tsx`, "Known" and "Seen" are counted over the
  active scope, while total reviews and accuracy are lifetime figures across
  the whole store. Either scope both or label which is which.
