...
- **Region frames fit shapes, not dots.** Samoa and Tonga sit across the antimeridian from the rest of Oceania; fitting them would make Oceania's filter frame the whole world. Every other dot is inside its continent's and subregion's frame. Recentring the projection east of Greenwich (the cut at 170°W) would bring those two home, and Fiji and Chukotka with them, but it moves every shape and label on the map, so it is not done here.
