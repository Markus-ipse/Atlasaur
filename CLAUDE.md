...
**Learner-facing names (#63).** The settings picker never shows the mechanism names ("Name → Click" and the rest stay developer vocabulary, in comments and tests). It is two rows, **Countries** / **Capitals**, each with **On the map** / **By typing**, and every option's example printed under it (`MODE_EXAMPLE` in `SettingsMenu.tsx`, a `Record<QuestionMode, string>`, also the option's `aria-describedby`), the chosen one in ink-deep. The examples are printed rather than tooltips so all four can be read before choosing, on touch too (choosing mid-round ends the card). Wherever scope is chosen (the settings' continent chips, the welcome's region picker) a `scopeLine` reads "South America · 12 countries" — "countries" even with territories on, by decision; `scopeOf` names only continents that have a chip, so a hidden Antarctica is never named, and past half the chips names what is left out ("Everywhere but Oceania").

`src/game/questionModes.ts` holds the only three predicates any of that hangs off — `factOf` (which record the mode grades), `isClickMode` (answered on the map) and `isTypedMode` (answered by typing; also exactly the modes that highlight the target). **Don't reintroduce string-literal mode checks**; adding a fifth mode should be a change to `QUESTION_MODES` and those three functions, plus the per-mode tables typed `Record<QuestionMode, …>` that the compiler then lists (the outcome log's codes, the settings picker's `MODE_EXAMPLE`), and nowhere else.
...
**Learner-facing model (R1.2 fold).** The learner never sees "Quiz" or "Study". Study is simply the app; a Quiz-mode pass is presented as a test round started from the Study summary's **"Test all N countries in <scope>"** door (`testDoorLabel` in `src/components/scopeSummary.ts`, which names the scope and its size; "Test all N countries" over the whole world), with its own `TestSummary` (Review N missed / Test again / Back to studying). There is no practice-mode toggle in the UI. Both axes stay in the reducer exactly as below — only the presentation changed. Don't reintroduce mode vocabulary in copy.

**Learner vocabulary: every country comes back.** The one idea the scheduler runs on is stated, fact-neutrally, in `StudyIntro` on the first Study miss ("Miss one and it's back in a few cards. Get it right and it comes back later …"); since #54 the `Welcome` no longer explains it. The intro's seen flag is `atlasaur:srs:seenIntro:v2`: the text was rewritten, so a learner who dismissed the old one sees the new one once. #54 only trimmed it and deliberately kept v2; bump the key again only for a rewrite worth reshowing. The copy that follows from it (`docs/plans/every-country-comes-back.md`): *coming back* is `dueCount` — never "to review" or "due"; **Back again** is the pill on a Study card that has come back (`cardIsReturning` in `useGame.ts`, derived from record existence, never stored) and on every card of a test's retry pass. It sits on the prompt, except in Name → Click (`hidesIntroduced` in `srs.ts`, the same rule that collapses the wash), where `ControlZone` moves it to the answer panels (`CorrectHero` / `RevealHero`): before the answer, "met before" would narrow "find X" to the known gold and the few learning cards the collapsed wash hides; *known* is the tile word. "Review" survives only on **Review N missed**. When the app says when the next card returns it goes through `nextBackLine` (`src/game/nextBack.ts`, over `srs.nextDueAt`, the strict complement of `dueCount`), exposed once as `GameApi.nextBack`; `nextBackOrNothing` is the fallback for a surface that always says something. Two truth rules bind new copy: "place" and "on your map" only where the fact is certainly `location` (the Welcome), and `dueCount === 0` is never an achievement — a missed card leaves the count for a few minutes — so caught-up copy says "nothing more has come back".

**Rounds of twelve.** `ROUND_SIZE = 12` (exported from `useGame.ts`). Round accounting lives in `State` as `roundCards` / `roundRight` / `roundNew` / `roundDone` / `roundsCompleted` and is advanced by `withRoundAdvance` inside `dismissFeedback` — a card counts when its feedback dismisses, so the correct flash or miss reveal always plays out first. When `roundCards` reaches `ROUND_SIZE` — or, in Study, when the next card would only be filler (see **A genuine finish** below) — `roundDone` flips and `App` renders `RoundBreak` (Keep going → `continueRound`, Done for now → `endSession`; Escape and the backdrop do nothing); `answer`/`skip` are ignored while it is up and the map is non-interactive. While any dialog is up (`modalOpen` in `App`: summary or round break — later PRs add their cards to it), the status bar, map and control zone are wrapped in an `inert` container, so keyboard focus cannot escape the dialog. `roundDone` is never set alongside `sessionDone` (the summary wins), but the round is still credited to `roundsCompleted`. A round is a presentation boundary only: FSRS picks, `retryQueue` and the review phase are untouched. `FRESH_ROUND` resets the counters on `continueRound`, `closeSummary`, `startReview` and `setPracticeMode`; `initialState` zeroes everything. Beside the round runs **the sitting** (`sittingCards` / `sittingRight` / `sittingNew`, advanced by `withSittingCard` from `withRoundAdvance` and from `endSession` for a card closed mid-reveal): every card since the summary last closed, which the Study summary reports as "This sitting: N of M right · K newly seen" — the only figure there that is not a standing or lifetime total. The wording comes from `tallyParts` (`src/components/tallyParts.ts`), shared with `RoundBreak`'s line so the two cannot drift, and "newly seen" from `cardIsNew`, the one definition both tallies use. `FRESH_STRETCH` (`FRESH_ROUND` plus `FRESH_SITTING`) resets round and sitting together on `initialState`, `closeSummary`, `setSpotlight`, `startReview`, `setPracticeMode` and `resetSrs`; `continueRound` and a test round's question-mode switch reset the round alone, and a round break, a question-mode switch and a scope change carry the sitting. "End session" is the **Done** button in the status bar, not in the gear.

**A genuine finish (#54, `docs/plans/a-genuine-finish.md`).** A Study round ends where the useful work does. `cardIsFiller(state, now)` (beside `cardIsReturning`, derived, never stored) is a card with a record for its fact, not due, and not in `studyResurfaceQueue` — on `pickNextStudy`'s partition, exactly the most-overdue fallback. `advanceCard` sets `roundEndedEarly` (in `FRESH_ROUND`) when the card it picked is filler, and `withRoundAdvance` treats that as a filled round: the break opens with its caught-up copy and `roundsCompleted` is credited, so a short round counts for the streak day and `roundsFinished`. Before ending, a pending miss is brought forward (`pickNextStudy` branch 2b). `fillerAccepted` (in `FRESH_SITTING`) is the learner's explicit Keep going anyway — set by `continueRound` from an early break or `acceptFiller` from the CaughtUp banner — and makes rounds run to twelve again for the rest of the sitting — except that a spent new-card allowance with unseen cards left still ends a round, and leaving a focus or changing scope clears it (the choice was for the pool as it was); without it each filler card would end its own round. A round is also held open while an in-pool miss is queued (`missWaiting`), so the last useful card's retry still comes back before the round ends. When the cap is what made the next card filler — `newCapReached`: `STUDY_NEW_CAP` used and unseen cards left in the pool — the break reads "10 new ones met." with Keep going as the default, and Keep going (the break's `continueRound` or the rest card's `closeSummary`) resets `newIntroducedThisStretch` and moves off the filler card. Every explicit Keep going resets it while unseen cards remain (`allowanceRefills`), not only once it is spent — and so do leaving a focus and a scope change (`withFreshAllowance`), so unseen countries they bring in are not held back by an allowance spent on the old pool — so the cap limits new cards per round rather than per sitting and a newcomer's second round is never cut off one card in; App's `caughtUp` excludes that case. Begin on a Today card with nothing waiting also accepts filler (its Escape and backdrop only close it, through `onDismiss`), and so does closing the rest card onto a filler card (`closeSummary`), so no screen's choice is asked twice. In a focus the banner and the break name the region and drop `nextBack`, which counts the whole scope. A caught-up break also carries the `CapitalsDoor` when `capitalOffer` has one — with the banner mostly skipped, it is where a learner has demonstrably run out of work — and it dispatches `setMode` before `continueRound`, which accepts filler only when it actually continues onto a filler card. The CaughtUp banner shows on `game.onlyFiller && !fillerAccepted` before a round's first card, never inside one. `nothingUseful` is judged over the pool, not the card on screen: the card is filler, no miss is queued (`missWaiting`, exposed as `game.missQueued`, which also keeps the rest card and App's `caughtUp` from saying "anyway"), and nothing else in the pool is due or allowed as new (`usefulInPool`). A scope change, leaving a focus or cards coming due can bring work in without replacing the card, so `applyScope` and `clearSpotlight` move off a filler card when something useful is waiting (`offFiller`), and `closeSummary` re-picks rather than resuming one. In a focus the banner and the caught-up break also offer **Back to all regions** (`clearSpotlight`, then `continueRound` on the break), since every focus now ends by running out. Done and Done for now land on the **rest card** (`StudySummary`): "That's it for now.", then "You got N right that you'd missed earlier." from `sittingRecovered` (keyed `${fact}:${iso3}` by the answer fact, read before the advance replaces `current` — a same-sitting recovery, never called known or remembered) above the sitting tally, so the ending leads with what changed rather than accuracy, "Kept in this browser — no account needed." only once there is a record to keep (`hasAnyRecord`) and the latest `saveStore` landed (it returns whether the write did; the wording claims where progress lives, not that every browser keeps it), one Keep going, and everything else under a closed `<details>` labelled "Figures, focus and tests". Focus lands on the dialog, and Escape and the backdrop do nothing, so the app stays at rest.

**Cross-day streak and Today card (R1.3).** `src/game/streak.ts` owns `atlasaur:streak:v1` (`{ version: 1, days: ["YYYY-MM-DD", …] }`): local calendar days on which the learner finished at least one round. `useGame` records today whenever `state.roundsCompleted` grows (`recordDay` returns the same store when today is already present, so the save effect stays quiet) and exposes `streak: StreakInfo` (`length`, `todayPlayed`, `day`) recomputed on the hourly/visibility `nowBucket` tick. `streakInfo` is forgiving by design: an unplayed today is neither counted nor held against the learner, and one missed day per rolling 7 days is bridged; two misses close together end the streak at the gap. Copy that reads it must never scold. "Erase all progress" (`resetSrs` on `GameApi`) clears the streak store too. The **Today card** (`TodayCard`) is hook state, not reducer state: it opens once per load for a learner with any SRS record (`showTodayCard` / `dismissTodayCard`), before the first prompt, and shows `N coming back · n new · Day d` with a single Begin (or, with nothing waiting, when the next ones come back). `RoundBreak`'s eyebrow reads `Round r · Day d` in Study; a test's break (`TestBreak`) reads `Test · A short break`.

**First-run welcome (R1.4).** `Welcome` opens once for a learner with no SRS records and no `atlasaur:seenWelcome` flag (decided at load in `useGame`, exposed as `showWelcome` / `dismissWelcome`; the flag is written on dismiss, or when a summary is reached under it). Existing learners upgrading past it never see it; "Erase all progress" clears the flag along with the SRS store and the streak, so a learner meets the app as a stranger again on the next load. It says what to do, not how the scheduler works (that is `StudyIntro`'s, on the first miss). One primary door, *Start a short round* (`setContinents(ALL_CONTINENTS)`), and two quieter text-style ones: *Pick a region* (inline `ContinentChip`s → `setContinents(picked)`) and *I know my way around — test me* (`setPracticeMode("quiz")`). `ContinentChip` is shared with the settings menu so the two scope pickers stay identical. The Today card and the welcome are mutually exclusive by construction (records present vs absent).
...
**A test is scored on first tries, in countries (#64).** `testTally(scope, completedSet, missedSet)` in `src/game/testTally.ts`, exposed once as `GameApi.testTally`, partitions the scope into *right first try* (found, never missed), *recovered* (missed, then found on a retry or in the review pass), *still missed* and *not yet asked*; the four always add up to the scope. The status bar's `ScorePanel` ("N done · R recovered · K to try again", deliberately no fraction, so it never reads as a second score beside First try), the test's round break and `TestSummary` all read it, and `testTallyParts` gives the break its line, so the three reconcile. Answers are not the unit — the retry queue can ask a country three times, and a percentage over answers once let "92% Right" sit beside a country still listed as missed; `score` / `total` stay in state but no test surface shows them. The summary's tiles are First try (`n/size`), Recovered and Still missed, with the rule said once under them; its list is "Missed at first", still-missed first, each recovered one marked. A test's twelve-card break is `TestBreak` in `RoundBreak.tsx`: "N of M done." (found, as the status bar counts it — never "N to go", which reads "185 to go" on a world test), the tally, Keep going as the default and **End the test here** (`endSession`) instead of Done for now — twelve answers are an intermission, never the test's end, which is the summary. In the review pass (`phase === "review"`) it reads "N misses left to review." with only recovered and to-try-again, since that pass asks nothing else. The summary's missed list is filtered to the scope, as the tally is.
...
- Pick precedence (in `pickNextStudy`, `src/game/pickCountry.ts`): in-session resurface (a recent miss whose gap has elapsed) → oldest due record → new country (by `notabilityTier` then `sizeTier` then iso3) subject to a soft cap of `STUDY_NEW_CAP = 10` new introductions per stretch → early retry (the soonest pending in-session miss, before its gap has elapsed) → most-overdue filler when nothing useful is left, which ends the round early unless the learner has chosen to keep going anyway (see **A genuine finish**).
...
- `newIntroducedThisStretch`, `studyResurfaceQueue`, and `studyStep` are volatile in-memory; they reset on `setPracticeMode("study")` and on reload, and the new-card count also resets on an explicit Keep going, on leaving a focus and on a scope change, while unseen cards remain (`allowanceRefills`).
- `state.sessionDone` is never auto-set in Study; the user exits via the status-bar **Done** button or the round break's "Done for now", which land on the Study rest card (`StudySummary`; lifetime stats under its disclosure). The summary's scoped tiles run Not yet seen → Seen → Known → Coming back (the settings' Data rows match). The counts overlap — Seen includes Known, and Coming back draws on both — so don't describe them as stages that add up to the total. Its doors each carry their own reason as a sub-line — there is no hint paragraph. Only **Keep going** sits on the card itself (calls `closeSummary`, which resumes on a card Done left unanswered — `resumeCurrent`, since the counts include it — and otherwise picks the next; nothing is focused, so Enter starts nothing; the sub-line says what comes next — what has come back, new cards (closing the summary refills a spent cap while unseen cards remain; App's `caughtUp` is false then), or, as **Keep going anyway**, when the next ones come back; during a focus it says "Still focusing on <subregion>" instead, since the counts are the whole scope's and the picks the region's). Under the **Figures, focus and tests** disclosure: **Focus on <subregion>** when a spotlight is offered ("N waiting there": countries unseen or due now in that subregion, from `askableBySubregion` rather than `masteryBySubregion`, so a focus always opens on useful work instead of a "nothing more" banner), then under "Or test yourself" **Test all N countries in <scope>** (`setPracticeMode("quiz")`) and the expedition door. Escape and backdrop click do nothing: the learner said Done. A focus is left from the status bar's chip, or from **Back to all regions** on the in-focus break and banner (`clearSpotlight`), which keeps the round and any staged grade, clears Keep going anyway, and keeps the card unless it is filler with something useful now waiting.
...
at its tenth answer, and, since #54, a Study round ended early because nothing
useful was left (`roundEndedEarly`).
...
card's exit (`App` wires "Back to studying" to `setPracticeMode("study")`;
since #54 Escape and the backdrop do nothing there, as on the rest card;
`closeSummary` is guarded the same way) and
