...
**Learner-facing names (#63).** The settings picker never shows the mechanism names ("Name → Click" and the rest stay developer vocabulary, in comments and tests). It is two rows, **Countries** / **Capitals**, each with **On the map** / **By typing**, and one example line for the chosen mode (`MODE_EXAMPLE` in `SettingsMenu.tsx`, a `Record<QuestionMode, string>`). Each option also carries its example as a `title`, so it can be read before choosing (choosing mid-round ends the card). Wherever scope is chosen (the settings' continent chips, the welcome's region picker) a `scopeLine` reads "South America · 12 countries" — "countries" even with territories on, by decision; `scopeOf` names only continents that have a chip, so a hidden Antarctica is never named, and past half the chips names what is left out ("Everywhere but Oceania").
