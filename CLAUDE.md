...
- `state.sessionDone` is never auto-set in Study; the user exits via the status-bar **Done** button or the round break's "Done for now", which land on a Study-flavored `SessionSummary` (lifetime stats). The summary's scoped tiles run Not yet seen → Seen → Known → Coming back (the settings' Data rows match). The counts overlap — Seen includes Known, and Coming back draws on both — so don't describe them as stages that add up to the total. Its doors come in two groups, each door carrying its own reason as a sub-line — there is no hint paragraph: **Focus on <subregion>** when a spotlight is offered ("N left to learn there"), **Keep going** (calls `closeSummary`, which resumes on a card Done left unanswered — `resumeCurrent`, since the counts include it — and otherwise picks the next; the default when no focus is offered; the sub-line says what comes next — what has come back, new cards while the stretch's cap allows (App's `caughtUp`, since closing the summary does not refill it), or, as **Keep going anyway**, when the next ones come back; during a focus it says "Still focusing on <subregion>" instead, since the counts are the whole scope's and the picks the region's), then under "Or test yourself" **Test me on these** (`setPracticeMode("quiz")`) and the expedition door. Escape and backdrop click both dismiss via `closeSummary`. A focus is left from the status bar's chip (`clearSpotlight`), which keeps the card, the round and any staged grade.
