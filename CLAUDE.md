...
- `setPracticeMode` (new) — resets only session counters (`score`/`streak`/`total`/`missed`/`autoGradePending`/`newIntroducedThisStretch`) plus the Study resurface state (`studyResurfaceQueue` → `[]`, `studyStep` → `0`). `retryQueue` and `completedSet` survive so a Study detour doesn't nuke a Quiz in-session review queue.
